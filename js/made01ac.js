...
;(function ($) {
  ...
  function pageFunctions() {
    ...
    var url =
      (~window.location.href.indexOf('local')
        ? 'http://localhost:5001'
        : 'https://ditto.money/api') + '/blog-posts'
    fetch(url)
      .then(function (response) {
        return response.json()
      })
      .then(function (posts) {
        var postsContainerEl = $('.recent-posts')
        postsContainerEl.css({display: 'block'})
        var postsEl = $('.recent-posts__list')
        posts.map(function (post) {
          postsEl.append(`
						<article class="post__card">
							<a href="${post.url}" class="post__link" target="_blank">
								<div class="post__image" style="background-image: url(${post.thumbnail})">
								</div>
								<time class="post__date label--small">${new Date(
                  post.date
                ).toLocaleDateString()}</time>
								<h5 class="post__title subtitle">${post.title}</h5>
							</a>
						</article>
					`)
        })
  ...
  // const $countdownEl = $('#countdown h3')
  // const end = new Date('2020-12-23')
  // end.setUTCHours(7) // 1500 SGT / 0700 GMT
  ...
  //   if (duration < 0) {
  //     cancelRaf(rafId)
  //     $countdownEl.text('LIVE NOW!').addClass('rainbow-animated')
  //     $('#countdown h2').hide()
  //     $('#countdown [data-hook=when]').hide()
  //     $('#countdown a').text('Buy Now').attr('href', '/crowdsale')
  //     return
  //   }
  ...
  //   $countdownEl.text(hours + ':' + minutes + ':' + seconds)

  //   rafId = raf(setCountdown)
  // }
